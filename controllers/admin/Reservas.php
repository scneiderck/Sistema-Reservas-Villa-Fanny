...
{
    if (isset($_POST) && is_numeric($idReserva)) {
        $data = $this->model->eliminarReserva($idReserva);
        if ($data) {
            $res = array('msg' => 'Reserva eliminada correctamente', 'type' => 'success');
        } else {
            $res = array('msg' => 'Error al eliminar la reserva', 'type' => 'error');
        }
    } else {
        $res = array('msg' => 'Error desconocido', 'type' => 'error');
    }
